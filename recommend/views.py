...
def recommend(request):
    ...
    #Privatize User Ratings to avoid exposing users data.
    ...
    for b in valuelist:
        b = b.tolist()
        b.insert(2, 0)
        newvaluelist.append(b)
        movieid.append(b[0])
        
    thevalues = []
    for a in newvaluelist:
        thevalues.append(a[1:])
    
    finalvalues = []
    for sumvalues in thevalues:
        x = BoundedSum(epsilon= 1.5, lower_bound = 12, upper_bound = 200, dtype ='int')
        sum_dp = x.quick_result(sumvalues)
        finalvalues.append(sum_dp)
    
    print(sum(finalvalues))
    i = 0
    combine = []
    while i < len(finalvalues):
        combine.append([movieid[i], finalvalues[i]])
        i+=1
    
    # print(combine)
     
    # user_filtered = [tuple(x) for x in user.values]
    user_filtered = [tuple(x) for x in combine]
    movie_id_watched = [each[0] for each in user_filtered]

    rec_no = random.randint(7,12)
    ...
    movie_list=list(Movie.objects.filter(id__in = movies_id_recommend).order_by(preserved)[:rec_no])
